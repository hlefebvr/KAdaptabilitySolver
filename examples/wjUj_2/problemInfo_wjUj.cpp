...
void KAdaptableInfo_wjUj::makeVars() {
    ...
    // x
    X.addVarType("x", 'B', 0, 1, m_instance->occurences().size());
...
void KAdaptableInfo_wjUj::makeConsX() {
    ...
    for (const JobOccurence *occ : m_instance->occurences()) {
        ConstraintExpression expr;
        expr.rowname("UB_x(" + std::to_string(occ->id) + ")");
        expr.addTermX(getVarIndex_1("x", occ->id), 1);
        expr.sign('G');
        expr.RHS(0);
        B_X.emplace_back(expr);
    }
    ...
    for (const auto& job : m_instance->jobs()) {
        ...
        constraints[job->id].sign('E');
        constraints[job->id].RHS(1);
    }

    for (const auto& occurrence : m_instance->occurences()) {
        if (occurrence->is_tardy_occurence) continue;
        constraints[occurrence->parent_job_id].addTermX(getVarIndex_1("x", occurrence->id), 1);
    }
...
void KAdaptableInfo_wjUj::makeConsY(unsigned int K) {
    ...
    for (unsigned int k = C_XY.size() ; k <= K ; ++k) {
        ...
        for (const auto& occurrence : m_instance->occurences()) {
            ...
            expr.addTermX(getVarIndex_1("x", occurrence->id), -1.);
            expr.sign('L');
            expr.RHS(0);
...
void KAdaptableInfo_wjUj::build() {
    ...
    numFirstStage = m_instance->jobs().size() + m_instance->occurences().size() + 1; // U, O, x
